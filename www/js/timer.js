    ...
    function makeEditable() {
        ...
        $el.on('blur', function() {
            ...
            if (val.indexOf('sec') > 0) {
                // sec
                totalSeconds = Number(val.replace(/\sсек/g, ''));
            } else if (val.indexOf('min') > 0) {
                // min
                val = val.replace(/\sмин/g, '');
                valArr = val.split(':');
    ...
    function secondsToTime(sec) {
        ...
        if (options.format) {
            ...
            if (timeObj.hours) {
                ...
                if (timeObj.minutes) {
                    time = timeObj.minutes + ':' + timeObj.seconds + ' мин.';
                } else {
                    time = timeObj.seconds + ' сек.';
                }
            }
        }
        return '<span class="glyphicon glyphicon-pause"></span>&nbsp;'+time;
    }
